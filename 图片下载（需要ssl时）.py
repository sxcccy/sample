...
import ssl
proxy_support = urllib.request.ProxyHandler({'http':'someProxy'})
...
for j in range(1,endPage):
  url='http://www.diaobishe.com/forum-2-'+str(j)+'.html'#页数的url地址
  print (url)
  html=urllib.request.urlopen(url).read()#读取首页的内容
  selector=etree.HTML(html)#转换为xml，用于在接下来识别
  links=selector.xpath('//div/a[@class="s xst"]/@href')#抓取当前页面的所有帖子的url
  #print (links)
  #print (j)
#大家可以使用浏览器自带的源码查看工具，在指定目标处查看元素，这样更快捷
 
  for i in links:
    url1=i#因为爬取到的地址是相对地址，所以要加上百度的domain
    html2=urllib.request.urlopen(url1).read()#读取当前页面的内容
    selector=etree.HTML(html2)#转换为xml用于识别
    link=selector.xpath('//img[@class="zoom"]/@file')#抓取图片，各位也可以更换为正则，或者其他你想要的内容
    #print (i)
#此处就是遍历下载
    for each in link:
      #print (each)
      print (u'正在下载%d'%k)
      fp=open('D:\\Python3\\apps\\Temp\\'+str(k)+'.jpg','wb')#下载在当前目录下 image文件夹内，图片格式为bmp
